...
class Admin_Model_Check_In_Report
{
    ...
    {
        global $wpdb;
        // $sql = "SELECT g.country AS code, 
        //  (SELECT COUNT(*) FROM $this->tbl_guests WHERE status = 1 AND country = g.country) AS register, 
        // COUNT(DISTINCT c.guests_ID) AS arrived
        // FROM $this->tbl_guests AS g
        // LEFT JOIN $this->tbl_check_in AS c ON g.ID = c.guests_ID AND c.event_ID = $this->current_event
        // WHERE g.status = 1 
        // GROUP BY g.country 
